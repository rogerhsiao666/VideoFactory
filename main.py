...
def _speedup_intro(src: str) -> str:
    """將片頭影片加速至 INTRO_SPEED 倍（音訊 + 畫面同步）。"""
    if INTRO_SPEED <= 1.001:
        return src
    ...
    cmd.extend([dst, "-loglevel", "error"])

    ret = subprocess.run(cmd)
    if ret.returncode != 0 or not os.path.exists(dst):
        print("⚠️  片頭加速失敗，改用原始片頭")
        return src
    return dst
...
""")
    ...
    if os.path.exists(active_intro):
        active_intro = _speedup_intro(active_intro)
